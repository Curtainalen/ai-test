...
from app.models import ExecutionStep,ExecutionTask,ProjectMember,UiExecutionStep,UiExecutionTask,User
from app.security import decode_access_token
from app.services.events import execution_channel, ui_execution_channel
from app.services.executions import task_view
from app.services.ui_runtime import execution_view as ui_execution_view
...
async def execution_ws(ws:WebSocket,project_id:str,execution_id:str):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user or not user.is_active or not member or not task: await ws.close(code=4403); return
...
@router.websocket("/ws/projects/{project_id}/ui-executions/{execution_id}")
async def ui_execution_ws(ws: WebSocket, project_id: str, execution_id: str):
    await ws.accept()
    try:
        auth_message = json.loads(await asyncio.wait_for(ws.receive_text(), timeout=5))
        user_id = decode_access_token(str(auth_message.get("token") or "")) if auth_message.get("type") == "auth" else ""
    ...
    async with AsyncSessionLocal() as db:
        ...
        member = await db.scalar(select(ProjectMember).where(ProjectMember.project_id == project_id, ProjectMember.user_id == user_id))
        task = await db.scalar(select(UiExecutionTask).where(UiExecutionTask.id == execution_id, UiExecutionTask.project_id == project_id))
        if not user or not user.is_active or not member or not task:
        ...
        snapshot = ui_execution_view(task, steps)
    await ws.send_text(json.dumps({"type": "snapshot", "version": snapshot["event_version"], "data": snapshot}, ensure_ascii=False))
    redis = Redis.from_url(get_settings().redis_url)
    pubsub = redis.pubsub()
    await pubsub.subscribe(ui_execution_channel(execution_id))
    try:
        while True:
            message = await pubsub.get_message(ignore_subscribe_messages=True, timeout=1.0)
            if message:
        ...
        await pubsub.unsubscribe(ui_execution_channel(execution_id))
        await pubsub.close()
        await redis.close()
