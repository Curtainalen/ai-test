...
def import_view(row): return {"id":row.id,"source_type":row.source_type,"source_name":row.source_name,"source_url":row.source_url,"spec_version":row.spec_version,"diff":row.diff,"warnings":row.warnings,"status":row.status,"revision":row.revision,"created_at":row.created_at.isoformat()}
...
async def create_import(db:AsyncSession,project_id:str,user:User,filename:str,content:bytes,source_type:str="file",source_url:str|None=None):
    await require_membership(db,project_id,user); parsed=parse_spec(content)
    source_key=source_url or filename
    existing=list((await db.scalars(select(ApiInterface).where(ApiInterface.project_id==project_id))).all())
    old=[{**(row.imported_snapshot or {}),"stable_key":row.stable_key,"manual_config":row.manual_config,"revision":row.revision} for row in existing if not row.is_deleted and (row.imported_snapshot or {}).get("source_key",(row.imported_snapshot or {}).get("source_name"))==source_key]
    incoming=[{**item,"stable_key":hashlib.sha256(f"{source_key}|{item['method']}|{item['normalized_path']}".encode()).hexdigest(),"source_name":filename[:512],"source_key":source_key} for item in parsed["interfaces"]]
