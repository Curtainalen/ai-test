...
async def _validate_refs(db,project_id,module_ids,case_ids,steps):
    ...
    if case_ids:
        cases=(await db.scalars(select(RequirementTestCase).where(RequirementTestCase.project_id==project_id, RequirementTestCase.id.in_(set(case_ids)), RequirementTestCase.status=="confirmed"))).all()
        if len(cases)!=len(set(case_ids)): raise AppError("TEST_CASE_NOT_CONFIRMED","场景只能关联本项目已确认测试用例",422)
...
async def create(db:AsyncSession,project_id:str,user:User,data):
    await require_membership(db,project_id,user); await _validate_refs(db,project_id,data.requirement_module_ids,data.requirement_test_case_ids,data.steps)
    row=TestScenario(project_id=project_id,name=data.name,description=data.description,scenario_type=data.scenario_type,priority=data.priority,requirement_module_ids=data.requirement_module_ids,requirement_test_case_ids=data.requirement_test_case_ids,created_by=user.id,status="draft")
...
async def update(db,project_id,user,scenario_id,data):
    ...
    await _validate_refs(db,project_id,data.requirement_module_ids,data.requirement_test_case_ids,data.steps)
    for key in ("name","description","scenario_type","priority","requirement_module_ids", "requirement_test_case_ids"): setattr(row,key,getattr(data,key))
