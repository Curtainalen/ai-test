...
async def create(db:AsyncSession,project_id:str,user:User,scenario_id:str,environment_id:str,idempotency_key:str):
    ...
    snapshot={"id":scenario.id,"name":scenario.name,"description":scenario.description,"version":scenario.version,"revision":scenario.revision,"requirement_module_ids":scenario.requirement_module_ids,"requirement_test_case_ids":getattr(scenario, "requirement_test_case_ids", []),"steps":[{"seq":s.seq,"name":s.name,"interface_id":s.interface_id,"request_override":s.request_override,"extracts":s.extracts,"assertions":s.assertions,"expected_result":s.expected_result,"timeout_ms":s.timeout_ms,"retry_count":s.retry_count,"continue_on_failure":s.continue_on_failure} for s in steps]}
