...
from app.models import ModelConfig, RequirementModule, RequirementReview, RequirementTestCase
...
def view(row):
    return {"id": row.id, "review_id": row.review_id, "document_version_id": row.document_version_id,
            ...
            "error_message": row.error_message, "reviewed_at": row.reviewed_at.isoformat() if row.reviewed_at else None}
...
async def generate(db, project_id, user, data):
    ...
    row = RequirementTestCase(project_id=project_id, document_version_id=(await db.get(RequirementModule, review.requirement_module_id)).document_version_id,
        requirement_module_id=review.requirement_module_id, review_id=review.id, model_config_id=config.id,
        stable_key=f"generation-{review.id[:8]}-{int(datetime.now(UTC).timestamp())}", title="正在生成测试用例候选",
        case_type="normal", preconditions=[], test_data_refs=[], steps=[], expected_result="", status="generating", created_by=user.id)
