...
def upgrade() -> None:
    ...
    if not sa.inspect(bind).has_table("document_images"):
    ...
    indexes = {item["name"] for item in sa.inspect(bind).get_indexes("document_images")}
    if "ix_document_images_project_id" not in indexes:
        op.create_index("ix_document_images_project_id", "document_images", ["project_id"])
    if "ix_document_images_document_version_id" not in indexes:
