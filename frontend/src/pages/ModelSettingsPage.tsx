  ...
  return <Space direction="vertical" className="page-block" size="large">
    ...
    <Modal className="model-settings-modal" open={open} width={760} title={editing ? '编辑模型配置' : '新增模型配置'} okText="保存" confirmLoading={saving} onOk={() => void save()} onCancel={() => setOpen(false)} destroyOnClose>
      <Form form={form} layout="vertical">
        ...
        <div className="model-settings-grid"><Form.Item label="协议" name="protocol" rules={[{ required: true }]}><Select options={protocolOptions} /></Form.Item><Form.Item label="模型名称" required><Space.Compact style={{ width: '100%' }}><Form.Item name="model_name" noStyle rules={[{ required: true, whitespace: true, max: 256 }]}><AutoComplete aria-label="模型名称" options={availableModels.map((model) => ({ value: model }))} placeholder="可手动输入，或先获取模型" style={{ flex: 1 }} /></Form.Item><Button loading={loadingModels} disabled={!editing} onClick={() => editing && void fetchModels(editing)}>获取模型</Button></Space.Compact></Form.Item></div>
