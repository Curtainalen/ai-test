import { CheckCircleOutlined, CloudServerOutlined, DeleteOutlined, EditOutlined, PlusOutlined } from '@ant-design/icons'
...
export function ModelSettingsPage() {
  ...
  const remove = async (row: ModelConfig) => { try { await api({ method: 'delete', url: `/settings/model-configs/${row.id}` }); await load(); message.success('模型配置已删除') } catch (error) { message.error((error as Error).message) } }
  ...
  return <Space direction="vertical" className="page-block" size="large">
    ...
    <Table rowKey="id" dataSource={rows} locale={{ emptyText: '暂无模型配置' }} columns={[
      ...
      { title: '操作', render: (_, row: ModelConfig) => <Space wrap><Button size="small" icon={<CloudServerOutlined />} onClick={() => void testSaved(row)}>测试连接</Button><Button size="small" icon={<EditOutlined />} onClick={() => openEdit(row)}>编辑</Button>{!row.is_default && <Popconfirm title="设为全局默认模型？" onConfirm={() => void setDefault(row)}><Button size="small" disabled={!row.is_enabled}>设为默认</Button></Popconfirm>}<Popconfirm title="确认删除模型配置？" onConfirm={() => void remove(row)} disabled={row.is_default}><Button size="small" danger icon={<DeleteOutlined />} disabled={row.is_default} aria-label={`删除模型配置 ${row.name}`} /></Popconfirm></Space> },
    ]} />
    <Modal className="model-settings-modal" open={open} width={760} title={editing ? '编辑模型配置' : '新增模型配置'} okText="保存" confirmLoading={saving} onOk={() => void save()} onCancel={() => setOpen(false)} destroyOnClose>
      <Form form={form} layout="vertical">
        ...
        <div className="model-settings-grid"><Form.Item label="协议" name="protocol" rules={[{ required: true }]}><Select options={protocolOptions} /></Form.Item><Form.Item label="模型名称" htmlFor="model_name" required><Space.Compact style={{ width: '100%' }}><Form.Item name="model_name" noStyle rules={[{ required: true, whitespace: true, max: 256 }]}><AutoComplete options={availableModels.map((model) => ({ value: model }))} placeholder="可手动输入，或先获取模型" style={{ flex: 1 }} /></Form.Item><Button loading={loadingModels} disabled={!editing} onClick={() => editing && void fetchModels(editing)}>获取模型</Button></Space.Compact></Form.Item></div>
